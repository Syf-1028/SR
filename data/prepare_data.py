...
def resize_and_convert(img, size, resample):
    if(img.size[0] != size):        # 检查是否需要缩放
        img = trans_fn.resize(img, size, resample)      # 缩放
        img = trans_fn.center_crop(img, size)           # 中心裁剪
    return img

# 图像转二进制（用于LMDB存储）
def image_convert_bytes(img):
    buffer = BytesIO()
    img.save(buffer, format='png')      # 保存PNG格式
    return buffer.getvalue()            # 返回二进制数据

# 多尺寸图像生成
def resize_multiple(img, sizes=(16, 128), resample=Image.BICUBIC, lmdb_save=False):
    lr_img = resize_and_convert(img, sizes[0], resample)    # 低分辨率图
    hr_img = resize_and_convert(img, sizes[1], resample)    # 高分辨率图
    sr_img = resize_and_convert(lr_img, sizes[1], resample) # 从低分辨率上采样的图
    ...
    return [lr_img, hr_img, sr_img] # 返回三种分辨率图像
...
def prepare_process_worker(wctx, file_subset):
    for file in file_subset:
        i, imgs = wctx.resize_fn(file)      # 处理单个文件
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', '-p', type=str,
                        default='D:\PycharmProjects\Image-Super-Resolution-via-Iterative-Refinement-master\dataset\celeba_hq_256\hr_128'.format(Path.home()))
    parser.add_argument('--out', '-o', type=str,
                        default='./dataset/celebahq')

    parser.add_argument('--size', type=str, default='16,128')   # 默认生成16×16和128×128
    ...
    args.out = '{}_{}_{}'.format(args.out, sizes[0], sizes[1])  # 输出路径添加尺寸后缀
